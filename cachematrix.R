...
## input,solve the inverse,cache them and display the same for the set() input matrix. 
## Additionally( although not asked in the exercise) the two functions together would also
...
## The get function helps in getting the value of the matrix as set by the set function, as and when required by 
...
makeCacheMatrix <- function(x = matrix()) {
	      sqmatrix <<-NULL							                                ## Parameter to determine square matrix
                list( set = function(y=matrix()){	                                ## Generates a list with 5 functions
                          invmat<<-NULL    
                          x<<-y
                          },
                          get = function() x,
                          cacheinvmat = function(invmatsolve) invmat<<-invmatsolve, ## Using lexical scoping rules 
                                                                                    ## and <<- operator for free variable
                                                                                    ## assignment and use
                          getcacheinvmat = function() invmat,
                          chksqmatrix= function(){									## This function helps to determine 
                          	                                                        ## whether the matrix given as 
                          	                                                        ## user input is square or not
                                    if(ncol(x)==nrow(x)){
                                              sqmatrix<<-NA
                                    }
                                    sqmatrix
...
## If the matrix is found to be square then the inverse if calculated and displayed only in case 
## that there is no cached inverse matix for the input matrix. Otherwise it goes ahead in the function 
## and just fethches and displays the previously cached value for the matrix.

cacheSolve <- function(x, ...) {
	z<-x$chksqmatrix()
          if(is.null(z)){															## Determine whether square matrix
